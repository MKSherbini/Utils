...
(function () {
    ...
    setTimeout(
        () => {
            createBtn("Cpp", CreateTemplate);
            createBtn(" Submit", runAutoResubmit);
        }, 5000
    )
    ...
    function getCases() {
        ...
        if (parents.length === 0)
            parents = $("strong").filter((i, e) => e.innerText.indexOf("Input") !== -1).map((i, e) => e.parentElement.parentElement);
    ...
    function getCases2() {
        let parents = $(".example-block");
        console.log(`parents length= ${parents.length}`);
        let cases = Array(parents.length);
        for (let i = 0; i < parents.length; i += 1) {
            console.log(`parents[${i}]= ${JSON.stringify(parents[i].textContent)}`);
            let ios = parents[i].querySelectorAll(".example-io");
            if (ios.length < 2) return cases;

            let input = targetCaseFromFilter(ios[0].innerText);
            let output = targetCaseFromFilter(ios[1].innerText);
            console.log(`case[${i}] input= ${JSON.stringify(input)}`);
            console.log(`case[${i}] output= ${JSON.stringify(output)}`);
            cases[i] = new TestCase(input, output);
            console.log(`case[${i}]= ${JSON.stringify(cases[i])}`);
    ...
    function CreateTemplate() {
        ...
        console.log(`cases[0]= ${cases[0]}`);
        if (cases.length === 0 || cases[0] === void 0) cases = getCases();
        if (cases !== void 0) {
            for (let i = 0; i < cases[0].inputs.length; i++) {
                let hasStrings = cases.some(c => {
                    return c.inputs[i].type.indexOf('string') !== -1;
                });
                if (hasStrings) {
                    cases.forEach((c) => {
                        c.inputs[i].type = c.inputs[i].type.replaceAll("char", "string");
                        c.inputs[i].value = c.inputs[i].value.replaceAll("'", "\"");
                    });
                }
            }
        }
...
//${/(.*)\/description/.exec(window.location.href)[1]}
