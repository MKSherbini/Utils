...
void Split(const std::string &iStr, std::vector<std::string> &oVec, std::string iLimiter)
{
    int idx = 0, lastIdx = 0;
    while ((idx = iStr.find(iLimiter, lastIdx)) != -1)
    {
        if (idx - lastIdx > 0)
        {
            oVec.push_back(iStr.substr(lastIdx, idx - lastIdx));
        }
        lastIdx = idx + iLimiter.size();
    }
    if (iStr.size() - lastIdx > 0)
    {
        oVec.push_back(iStr.substr(lastIdx, iStr.size() - lastIdx));
    }
}
